...
import torch
from torch import native_group_norm, nn
...
class TextLSTMClassifier(nn.Module):
    ...
    def __init__(self, vocab_size:int, embed_dim:int, hidden_dim:int, num_classes:int) -> None:
        ...
        self.lstm = nn.LSTM(embed_dim, hidden_dim, num_layers=2, batch_first=True) # 문장 순서를 고려하여 정보를 누적하는 LSTM 계층이다.
        self.dropout = nn.Dropout(p=0.5) # 과적합을 줄이기 위해 일부 뉴련 출력을 무작위로 0으로 만든다.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        logits = self.fc(dropped) # 각 클래스에 대한 원시 점수(logit)을 계산한다.
